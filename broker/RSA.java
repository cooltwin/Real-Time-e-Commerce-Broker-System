...
public class RSA {
	...
	public RSA(String webserver_name){
		this.client = client;
	}
	...
	public byte[] unsign_message(byte[] signedMessage)
	{
		MiscellaneousWork mw = new MiscellaneousWork();
		try
		{
	...
	public PublicKey readPublicKeyFromFile() throws IOException{
		MiscellaneousWork mw = new MiscellaneousWork();
		try {
			BigInteger modulus =  new BigInteger(mw.read_password_file("client", "PublicKey Modulus"));
			BigInteger exponent = new BigInteger(mw.read_password_file("client", "PublicKey Exponent")); 
	...
	public PrivateKey readPrivateKeyFromFile() throws IOException{
		...
		try {
			...
			BigInteger modulus = new BigInteger(mw.read_password_file("My_Machine_Details", "MyPrivateKey Modulus"));
			BigInteger exponent = new BigInteger(mw.read_password_file("My_Machine_Details", "MyPrivateKey Exponent")); 
